...
if (isset($_POST['submit'])) {
    ...
    $stmt = $bd->prepare("SELECT password FROM users WHERE email = ?");
    ...
    if ($user && md5($currentPassword) === $user['password']) {
        ...
        if ($newPassword === $confirmPassword) {
            // Update the password
            $stmt = $bd->prepare("UPDATE users SET password = ?, updationDate = ? WHERE email = ?");
            $stmt->bind_param("sss", md5($newPassword), $currentTime, $_SESSION['login']);
            if ($stmt->execute()) {
