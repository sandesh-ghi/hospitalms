...
if (isset($_POST['submit'])) {
    ...
    $fname = $bd->real_escape_string($_POST['fname']);
    $address = $bd->real_escape_string($_POST['address']);
    $city = $bd->real_escape_string($_POST['city']);
    $gender = $bd->real_escape_string($_POST['gender']);
    $email = $_SESSION['login'];

    // Prepare and execute SQL statement to update profile
    $stmt = $bd->prepare("UPDATE users SET fullName = ?, address = ?, city = ?, gender = ? WHERE email = ?");
    $stmt->bind_param("sssss", $fname, $address, $city, $gender, $email);
    if ($stmt->execute()) {
