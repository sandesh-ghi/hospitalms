...
if (isset($_POST['submit'])) {
    ...
    if ($num) {
        ...
        // Log user login
        $log_stmt = $bd->prepare("INSERT INTO userlog (uid, username, userip, status) VALUES (?, ?, ?, ?)");
        ...
        // Log failed login
        $log_stmt = $bd->prepare("INSERT INTO userlog (username, userip, status) VALUES (?, ?, ?)");
        $log_stmt->bind_param("ssi", $_SESSION['login'], $uip, $status);
        $log_stmt->execute();
